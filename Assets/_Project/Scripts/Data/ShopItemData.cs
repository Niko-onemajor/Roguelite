...
namespace Roguelite
{
    ...
    public enum PassiveType
    {
        ...
        // ── 符文机制(白银/龙魂/黄金/棱彩) ──
        Perishable,      // 易损：灼烧 DoT 可暴击(暴击时伤害×2)，自带+20%暴击
        ParryDefense,    // 会心防守：受击时按暴击几率减免伤害(减伤=暴击×20%)
        Swiftness,       // 唯快不破：玩家移速高于目标时普攻附加+15%伤害
        HeavyHitter,     // 重量级打击手：普攻附加 最大生命4% 物理伤害
        Eroding,         // 侵蚀：普攻削减目标护甲(破甲5点, 4秒)
        LightStrike,     // 点亮：每第4次普攻发射 4 枚魔法飞弹(法强×0.6)
        Executioner,     // 裁决使：对生命<50%敌人伤害+15%
        InfernoSoul,     // 炼狱龙魂：每5秒一次 90+12%攻击力+6%法强 范围爆炸(冷却5秒)
        MountainSoul,    // 山脉龙魂：脱离战斗5秒后获得护盾(上限最大生命10%)
        OceanSoul,       // 海洋龙魂：普攻命中回复爱与法力
        ArcaneVamp,      // 术士果汁盒：按法强获得法术吸血(每100法强3.5%, 上限15%)
        UnholyMastery,   // 超凡邪恶：技能命中永久+1法强(上限30)
        MagicMissile,    // 魔法飞弹：技能命中附 目标最大生命5% 真实伤害
        Porcupine,       // 豪猪尖刺：受击叠尖刺,8层爆发伤害并减速周围敌人
        TankGrowth,      // 坦克引擎：击杀后永久+20最大生命
        ToothTally,      // 牙仙子：击杀永久+0.5护甲穿/法穿(上限15)
        ShrinkBoost,     // 缩小引擎：击杀后短暂提升移速
        PearledFist,     // 珠光护手：技能伤害可暴击(145%总伤害)
        TwinBlade,       // 双刀流：普攻对附近另一敌人发射40%伤害次级箭矢
